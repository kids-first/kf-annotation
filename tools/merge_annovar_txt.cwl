...
arguments:
  - position: 1
    ...
    valueFrom: >-
      ...
      ls -d $(inputs.input_anno[0].dirname)/* | xargs -IFN zcat FN | grep -vE "^Chr" | bgzip -c >> $(inputs.output_basename).$(inputs.tool_name).$(inputs.protocol_name).hg38_multianno.txt.gz

inputs:
    input_anno: {type: 'File[]', doc: "List of files from annovar scatter job"}
    header_str: {type: string?, default: "Chr\tStart\tEnd\tRef\tAlt\tFunc.refGene\tGene.refGene\tGeneDetail.refGene\tExonicFunc.refGene\tAAChange.refGene\tOtherinfo\n", doc: "anticipated header of annovar table. basically a hack to avoid guessing/parsing the file"}
